...
void hueDino::getSensorLastMovement(void) {
    StaticJsonDocument<4096> jsonBuffer;
    String json = "";

    Serial.println(_getLight(13));
    json = _getSensor(47);
    Serial.println(json);
    ...
    if (error) {
        ...
        String lastPresence = jsonBuffer["state:lastupdated"].as<String>();
        Serial.println(lastPresence);
    }
    /**
    {
            "state": {
                    "presence": false,
                    "lastupdated": "2022-01-09T14:51:43"
            },
            "swupdate": {
                    "state": "noupdates",
                    "lastinstall": "2021-09-09T18:32:17"
            },
            "config": {
                    "on": true,
                    "battery": 100,
            ...
            "uniqueid": "00:17:88:01:09:18:fe:46-02-0406",
            "capabilities": {
                    "certified": true,
                    "primary": true
            }
...
String hueDino::_getLights(void) {
    ...
    // _restclient.get(endpoint);
    _restclient.get("/api/"+String(_userId)+"/lights/14");

    return _restclient.readResponse();
}

String hueDino::_getSensor(uint8_t sensorId) {
    String endpoint = "/api/";
...
String hueDino::_getLight(uint8_t lightId) {
    String endpoint = "/api/";
